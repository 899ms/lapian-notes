...
export async function fetchAutoSubtitle(name: string, duration: number, signal?: AbortSignal): Promise<AutoSubtitleResult | null> {
  ...
  if (subtitles.length < 50) return null
  return { subtitles, filename: data.filename, source: data.source ?? data.filename }
}
